...
from database import main_driver
...
class EquipmentType(Resource):
    ...
    def post(self):
        ...
        try:
            if operation == 'create':
                ...
                duplicate_check_query = main_driver.db.equipment_type.find_one(filter={"name": data.get('name')})
                if duplicate_check_query is not None:
                    raise DuplicateResourceCreationError(data.get('name'), 'EquipmentType')
                new_id = main_driver.db.equipment_type.insert_one(data).inserted_id
                ...
                results = []
                for result in main_driver.db.equipment_type.find(filter=criteria, limit=limit):
    ...
    @resource_auth.login_required
    def get(self, obj_id):
        result = main_driver.db.equipment_type.find_one({"_id": ObjectId(obj_id)})
        if result is None:
            return result
    ...
    def delete(self, obj_id):
        result = main_driver.db.equipment_type.delete_one({"_id": ObjectId(obj_id)})
...
class Equipment(Resource):
    ...
    @resource_auth.login_required
    def delete(self, obj_id):
        result = main_driver.db.equipment.delete_one({"_id": ObjectId(obj_id)})
        return result.raw_result

    @resource_auth.login_required
    def get(self, obj_id):
        result = main_driver.db.equipment.find_one({"_id": ObjectId(obj_id)})
        if result is None:
            return result
    ...
    def post(self):
        ...
        try:
            if operation == 'create':
                ...
                duplicate_check_query = main_driver.db.equipment.find_one(filter={"name": data.get('name')})
                if duplicate_check_query is not None:
                    raise DuplicateResourceCreationError(data.get('name'), 'Equipment')
                type_result = main_driver.db.equipment_type.find_one({"name": equipment_type})
                if type_result is not None:
                    data['type'] = type_result['_id']
                else:
                    data['type'] = main_driver.db.equipment_type.insert_one({"name": equipment_type}).inserted_id
                new_equipment = main_driver.db.equipment.insert_one(data)
                ...
                if name_field is not None and type_field is not None:
                    type_results = main_driver.db.equipment_type.find(filter={"name": utils.translate_query(type_field)})
                ...
                results = []
                for raw_result in main_driver.db.equipment.find(filter=criteria, limit=limit):
                    result = utils.bson_to_json(raw_result)
                    equipment_type_id = raw_result['type']
                    equipment_type = main_driver.db.equipment_type.find_one(filter={"_id": equipment_type_id})
                    del result['type']
...
class Muscle(Resource):
    ...
    @resource_auth.login_required
    def delete(self, obj_id):
        result = main_driver.db.muscle.delete_one({"_id": ObjectId(obj_id)})
        return result.raw_result

    @resource_auth.login_required
    def get(self, obj_id):
        result = main_driver.db.muscle.find_one({"_id": ObjectId(obj_id)})
        if result is None:
            return result
        return utils.bson_to_json(json.loads(dumps(result)))
        ...
        try:
            if operation == 'create':
                ...
                if data.get('imageURLs') is None:
                    data.update({'imageURLs': []})
                duplicate_check_query = main_driver.db.muscle.find_one(filter={"name": data.get('name')})
                if duplicate_check_query is not None:
                    raise DuplicateResourceCreationError(data.get('name'), 'Muscle')
                new_id = main_driver.db.muscle.insert_one(data).inserted_id
                return json.loads(str(Response(success=True, data=str(new_id))))
            elif operation == 'update':
                raise NotImplementedError
            elif operation == 'query':
                ...
                if name_field is not None:
                    criteria = {"name": utils.translate_query(name_field)}
                results = []
                for result in main_driver.db.muscle.find(filter=criteria, limit=limit):
                    results.append(utils.bson_to_json(result))
                LOGGER.debug('response sent %s', str(Response(success=True, data=results)))
                return json.loads(str(Response(success=True, data=results)))
        except InvalidRequestError as e:
...
class MuscleGroup(Resource):
    ...
    @resource_auth.login_required
    def delete(self, obj_id):
        result = main_driver.db.muscle_group.delete_one({"_id": ObjectId(obj_id)})
        return result.raw_result

    @resource_auth.login_required
    def get(self, obj_id):
        result = main_driver.db.muscle_group.find_one({"_id": ObjectId(obj_id)})
        if result is None:
            return result
        return utils.bson_to_json(json.loads(dumps(result)))
    ...
    def post(self):
        ...
        try:
            if operation == 'create':
                ...
                if data.get('imageURLs') is None:
                    data.update({'imageURLs': []})
                duplicate_check_query = main_driver.db.muscle_group.find_one(filter={"name": data.get('name')})
                if duplicate_check_query is not None:
                    raise DuplicateResourceCreationError(data.get('name'), 'MuscleGroup')
                muscles = data['muscles']
                muscle_ids_to_save = []
                for muscle in muscles:
                    muscle_query = main_driver.db.muscle.find_one(filter={"name": muscle})
                    if muscle_query is not None:
                        muscle_ids_to_save.append(muscle_query['_id'])
                    else:
                        new_muscle = {"name": muscle, "imageURLs": []}
                        muscle_ids_to_save.append(main_driver.db.muscle.insert_one(new_muscle).inserted_id)
                data['muscles'] = muscle_ids_to_save
                new_id = main_driver.db.muscle_group.insert_one(data).inserted_id
                return json.loads(str(Response(success=True, data=str(new_id))))
            elif operation == 'update':
                raise NotImplementedError
            elif operation == 'query':
                ...
                if name_field is not None:
                    ...
                    for muscle in muscles_field:
                        muscle_result = main_driver.db.muscle.find_one(filter={"name": muscle})
                ...
                results = []
                for raw_result in main_driver.db.muscle_group.find(filter=criteria, limit=limit):
...
class ExerciseMetric(Resource):
    ...
    def post(self):
        ...
        if operation == 'create':
            try:
                ...
                new_id = main_driver.db.exercise_metric.insert_one(data).inserted_id
                return json.loads(str(Response(success=True, data=str(new_id))))
            except InvalidResourceCreationError as e:
                return json.loads(str(ErrorResponse(e)))
            ...
            if name_field is not None:
                criteria = {"name": utils.translate_query(name_field)}
            results = []
            for result in main_driver.db.exercise_metric.find(filter=criteria, limit=limit):
                results.append(utils.bson_to_json(result))
            return json.loads(str(Response(success=True, data=results)))

    @resource_auth.login_required
    def get(self, obj_id):
        result = main_driver.db.exercise_metric.find_one({"_id": ObjectId(obj_id)})
        if result is None:
            return result
        return utils.bson_to_json(result)
    ...
    @resource_auth.login_required
    def delete(self, obj_id):
        result = main_driver.db.exercise_metric.delete_one({"_id": ObjectId(obj_id)})
        return result.raw_result
...
class CategoryTag(Resource):
    ...
    def post(self):
        try:
            ...
            if operation == 'create':
                ...
                new_id = main_driver.db.category_type.insert_one(data).inserted_id
                return json.loads(str(Response(success=True, data=str(new_id))))
                ...
                if name_field is not None:
                    criteria = {"name": utils.translate_query(name_field)}
                results = []
                for result in main_driver.db.category_type.find(filter=criteria, limit=limit):
                    results.append(utils.bson_to_json(result))
                return json.loads(str(Response(success=True, data=results)))
            else:
    ...
    @resource_auth.login_required
    def get(self, obj_id):
        result = main_driver.db.category_type.find_one({"_id": ObjectId(obj_id)})
        if result is None:
            return result
        return utils.bson_to_json(result)
    ...
    @resource_auth.login_required
    def delete(self, obj_id):
        result = main_driver.db.category_type.delete_one({"_id": ObjectId(obj_id)})
        return result.raw_result
...
class Exercise(Resource):
            ...
            if operation == 'create':
                ...
                if data.get('equipments') is None:
                    ...
                    for equipment in equipments:
                        equipment_query = main_driver.db.equipment.find_one(filter={"name": equipment})
                ...
                duplicate_check_query = main_driver.db.exercise.find_one(filter={"name": data.get('name')})
                if duplicate_check_query is not None:
                ...
                new_id = main_driver.db.exercise.insert_one(data).inserted_id
                return json.loads(str(Response(success=True, data=str(new_id))))
                ...
                if keyword_field is not None:
                    ...
                    muscle_result = main_driver.db.muscle.find(filter={"$or": subquery}, projection={"_id": 1})
                    equipment_result = main_driver.db.equipment.find(filter={"$or": subquery}, projection={"_id": 1})
                    ...
                    for raw_result in main_driver.db.exercise.find(filter=criteria, limit=limit):
    ...
    @resource_auth.login_required
    def get(self, obj_id):
        result = main_driver.db.exercise.find_one({"_id": ObjectId(obj_id)})
        if result is None:
            return result
        return utils.bson_to_json(result)
    ...
    @resource_auth.login_required
    def delete(self, obj_id):
        result = main_driver.db.exercise.delete_one({"_id": ObjectId(obj_id)})
        return result.raw_result
...
def get_muscles_for_result(raw_result, param_name):
    ...
    for muscle_id in raw_result[param_name]:
        muscles.append(utils.bson_to_json(main_driver.db.muscle.find_one(filter={"_id": muscle_id})))
    return muscles
...
def get_equipments_for_result(raw_result, param_name):
    ...
    for equipment_id in raw_result[param_name]:
        equipments.append(utils.bson_to_json(main_driver.db.equipment.find_one(filter={"_id": equipment_id})))
    return equipments
